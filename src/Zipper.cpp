...
enum class zipper_error { NO_ERROR = 0, OPENING_ERROR, INTERNAL_ERROR, NO_ENTRY, SECURITY_ERROR };

// *************************************************************************
//! \brief std::error_code instead of throw() or errno.
// *************************************************************************
struct ZipperErrorCategory : std::error_category {
    virtual const char* name() const noexcept override { return "zipper"; }
    ...
    virtual std::string message(int ev) const override
    {
        ...
        switch (static_cast<zipper_error>(ev)) {
            case zipper_error::NO_ERROR: return "There was no error";
            case zipper_error::OPENING_ERROR: return "Opening error";
            case zipper_error::INTERNAL_ERROR: return "Internal error";
            case zipper_error::NO_ENTRY: return "Error, couldn't get the current entry info";
            case zipper_error::SECURITY_ERROR: return "ZipSlip security";
...
struct Zipper::Impl {
    ...
    bool initFile(const std::string& filename, Zipper::openFlags flags)
    {
        ...
        if (Path::exist(filename)) {
            if (!Path::isFile(filename)) {
                m_error_code = make_error_code(zipper_error::OPENING_ERROR, "Is a directory");
                return false;
            }
        ...
        m_error_code = make_error_code(zipper_error::OPENING_ERROR, strerror(errno));
        return false;
    }
    ...
    // -------------------------------------------------------------------------
    bool add(std::istream& input_stream, const std::tm& timestamp, const std::string& nameInZip,
             const std::string& password, int flags)
    {
        if (!m_zf) {
            m_error_code = make_error_code(zipper_error::INTERNAL_ERROR);
            return false;
        ...
        if (canonNameInZip.find_first_of("..") == 0u) {
            ...
            str << "Security error: forbidden insertion of " << nameInZip << " (canonic: " << canonNameInZip
        ...
        if (password.empty()) {
            err = zipOpenNewFileInZip64(m_zf, canonNameInZip.c_str(), &zi, nullptr, 0, nullptr, 0, nullptr /* comment*/,
                                        (compressLevel != 0) ? Z_DEFLATED : 0, compressLevel, zip64);
        } else {
            getFileCrc(input_stream, buff, crcFile);
            err = zipOpenNewFileInZip3_64(m_zf, canonNameInZip.c_str(), &zi, nullptr, 0, nullptr, 0,
                                          nullptr /* comment*/, (compressLevel != 0) ? Z_DEFLATED : 0, compressLevel, 0,
                                          /* -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, */
                                          -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, password.c_str(), crcFile,
                                          zip64);
        }
        ...
        if (ZIP_OK == err) {
            do {
                ...
                if (size_read < buff.size() && !input_stream.eof() && !input_stream.good()) { err = ZIP_ERRNO; }
...
void Zipper::release()
{
    ...
    if (!m_usingStream) { delete &m_obuffer; }
...
{
    if (m_impl == nullptr) {
        m_error_code = make_error_code(zipper_error::INTERNAL_ERROR, "Malloc error");
        return false;
    }
    ...
    if (m_usingMemoryVector) {
        if (!m_impl->initWithVector(m_vecbuffer)) return false;
    } else if (m_usingStream) {
        if (!m_impl->initWithStream(m_obuffer)) return false;
    } else {
        if (!m_impl->initFile(m_zipname, flags)) return false;
    }
